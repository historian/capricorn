...
get(resource) ->
  case filename:split(THIS:get(path)) of
  ["/" | Path] -> Path;
  Else         -> Else
  end;
get(range) ->
...
respond({Code, ResponseHeaders, chunked}) ->
    ...
    start_response({Code, HResponse1});
respond({Code, ResponseHeaders, Template, Args}) ->
  Body = io_lib:format(Template, Args),
...
ok({ContentType, Body}) ->
    ok({ContentType, [], Body});
ok({ContentType, [A|_]=Template, Args}) when is_integer(A) ->
    Body = io_lib:format(Template, Args),
    ok({ContentType, Body});
